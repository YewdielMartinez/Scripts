...
function setupFtp {
    ...
    # Eliminar permisos existentes de "Everyone" si los hubiera
    $acl.Access | Where-Object { $_.IdentityReference -eq "Everyone" } | ForEach-Object { 
        $acl.RemoveAccessRule($_) 
    }

    # Asignar permisos de solo lectura a "Everyone"
    $permissionEveryone = New-Object System.Security.AccessControl.FileSystemAccessRule("Everyone", "Read", "ContainerInherit,ObjectInherit", "None", "Allow")
    $acl.AddAccessRule($permissionEveryone)

    # Asegurar que el usuario anónimo de IIS (IUSR) solo tenga lectura
    $anonUser = "IUSR"
    ...
    # Configurar IIS con las rutas y propiedades adecuadas
    ...
    # Activar autenticación básica y anónima
    ...
    PrintMessage "success" "Creación de carpetas y configuraciones terminado."
}
...
function getPassword {
    do {
        $password = InputText "Contraseña del usuario"  
...
function getGroup {
    ...
    do{
        ...
        switch ($choice) {
            ...
            default { 
                PrintMessage "error" "Opcion inválida, intente nuevamente."
                continue
            }
        }
...
function setupUsers {
    ...
    for ($j = 1; $j -le $userCount; $j++) {
        ...
        $confirmInput = InputText "Seguro que quiere crear al usuario? [Y/N]"
    ...
    PrintMessage "success" "Usuarios configurados perfestamente."
}
...
function mainMenu {
    do {
        Clear-Host
        Write-Host -ForegroundColor Cyan "------Menu de configuracion de FTP------"
        Write-Host -ForegroundColor Green "1) Instalar y configurar FTP"
        Write-Host -ForegroundColor Green "2) Configurar usuarios"
        Write-Host -ForegroundColor Green "3) Salir"
        ...
        switch ($choice) {
            ...
            default {
                Write-Host -ForegroundColor Red "Opcion invalida, intente nuevamente."
            }
        }
    } while ($choice -ne 3)
}
